...
void destroy_pool(SELF) {
	...
	while (self->scope_depth) {
		fprintf(stderr,
				"libcmemman: warning: scope %d never exits until destroying "
				"the pool.",
				self->scope_depth);
		mp_exit_scope(self);
	}
	...
	assert(self != NULL && self->ptr_pool != NULL);
	free(self->ptr_pool);
	free(self);
}
...
void mp_enter_scope(SELF) {
	...
	if (self->scope_depth == 0) {
		...
		// Increase the scope arrays and add the element to the end of the
		// array, i.e., push a new scope.
		self->scope_depth++;
		self->ptr_pool = (ptr_info_t **)realloc(
			self->ptr_pool, sizeof(ptr_info_t *) * self->scope_depth);
		self->ptr_pool[self->scope_depth - 1] =
			(ptr_info_t *)malloc(sizeof(ptr_info_t));
...
int prv_mp_get_size_of_scope(SELF, int scope_id) {
	int c = 0;
	while (self->ptr_pool[scope_id][c].ptr)
		c++;
	return c;
}
...
void prv_mp_add_ptr_to_scope(SELF, int scope_id, void *ptr) {
	...
	self->ptr_pool[scope_id] = (ptr_info_t *)realloc(
		self->ptr_pool[scope_id], (len + 2) * sizeof(ptr_info_t));
...
void prv_mp_remove_ptr_from_scope(SELF, int scope_id, void *ptr) {
	...
	if (idx == -1) {
		fprintf(stderr,
				"libmemman: warning: trying to remove pointer (%p) that "
				"doesn't exist in scope %d",
				ptr, scope_id);
		return;
	}
	...
	for (int i = idx; i < len; i++) {
		...
		self->ptr_pool[scope_id][i].destructor =
			self->ptr_pool[scope_id][i + 1].destructor;
	}
}
...
void mp_exit_scope(SELF) {
	...
	for (int i = 0; i < len; i++) {
		...
		if (self->ptr_pool[self->scope_depth - 1][i].destructor != NULL) {
			self->ptr_pool[self->scope_depth - 1][i].destructor(
				self->ptr_pool[self->scope_depth - 1][i].ptr);
		}
		// Free the pointer.
...
void *mp_malloc(SELF, size_t size) {
	if (self->scope_depth == 0) {
		fprintf(stderr, "libcmemman: fatal: trying to allocate memory without "
						"entering any scope.");
		destroy_pool(self);
		exit(-1);
	}
	...
	if (!result) {
		// If failed, just return NULL and do nothing because we needn't
		// register NULL.
		return NULL;
	}
